...
说明：`deploy/docker/base-ubuntu1804`、`skydds-runtime-ubuntu1804` 以及对应镜像 tag / 归档文件名均对应 Ubuntu 18.04 arm64 基线。

- SkyDDS 安装包目录（压缩包）：`third_party/skydds/packages/`
- SkyDDS SDK 解压目录：`third_party/skydds/sdk/`
- 离线基础镜像归档（必须保留）：`deploy/images/forward-stub-base-ubuntu1804-arm64.tar.gz`
...
### 2.1 Base（Ubuntu 18.04，基础环境镜像）

- Dockerfile: `deploy/docker/base-ubuntu1804/Dockerfile`
- Build&Save: `deploy/docker/build-and-save-base-ubuntu1804.sh`
- Load: `deploy/docker/load-base-ubuntu1804.sh`
- 默认标签: `forward-stub-base:ubuntu1804-arm64`
- 默认归档: `deploy/images/forward-stub-base-ubuntu1804-arm64.tar.gz`
- 默认目标平台: `linux/arm64`（即 `aarch64`）

### 2.2 SkyDDS Runtime（Ubuntu 18.04，主服务镜像）

- Dockerfile: `deploy/docker/skydds-runtime-ubuntu1804/Dockerfile`
- Build&Save: `deploy/docker/build-and-save-skydds-runtime-ubuntu1804.sh`
- Load: `deploy/docker/load-skydds-runtime-ubuntu1804.sh`
- 默认标签: `forward-stub:skydds-ubuntu1804-runtime-arm64`
- 默认归档: `deploy/images/forward-stub-skydds-ubuntu1804-runtime-arm64.tar.gz`
- 默认目标平台: `linux/arm64`（即 `aarch64`）

该 Dockerfile 在构建阶段会：

0. 基于 `forward-stub-base:ubuntu1804-arm64`（继承基础构建环境与 tcpdump）。
...
```bash
./deploy/docker/build-and-save-base-ubuntu1804.sh
```

如需显式覆盖平台，可传入：

```bash
PLATFORM=linux/arm64 ./deploy/docker/build-and-save-base-ubuntu1804.sh
```
...
运行时镜像默认同样按 `linux/arm64` 构建，并要求本地已先存在同平台的 `forward-stub-base:ubuntu1804-arm64`。

### 3.3 导入离线归档

```bash
./deploy/docker/load-base-ubuntu1804.sh
./deploy/docker/load-skydds-runtime-ubuntu1804.sh
```
