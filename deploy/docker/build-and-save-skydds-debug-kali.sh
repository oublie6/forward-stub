...
IMAGE_TAG="${IMAGE_TAG:-forward-stub:skydds-kali-debug}"
OUTPUT_FILE="${OUTPUT_FILE:-${IMAGES_DIR}/forward-stub-skydds-kali-debug.tar.gz}"
...
tmp_tar=$(mktemp "${TMPDIR:-/tmp}/forward-stub-skydds-kali-debug.XXXXXX.tar")
cleanup() {
  rm -f "${tmp_tar}"
}
trap cleanup EXIT

echo "[INFO] Building SkyDDS debug image (Kali)"
