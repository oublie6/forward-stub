...
- `record_key_source` 当前只支持：`payload`、`match_key`、`kafka_record_key`、`remote`、`local`、`file_name`、`file_path`、`transfer_id`、`route_sender`。热重载时若这些字段变化，会重建 sender，并联动重建引用它的 task。
...
- `kafka_record_key`：pipeline 写入的专用 Kafka record key 字段。
...
Kafka 专用 record key 示例：

```json
{
  "type": "kafka",
  "partitioner": "hash_key",
  "record_key_source": "kafka_record_key"
}
```
...
| `length` | int | `set_kafka_record_key_from_offset_bytes` | 从 payload 读取的固定字节数，必须 `>0`。 |
| `encoding` | string | `set_kafka_record_key_from_offset_bytes` | 写入 `KafkaRecordKey` 前的编码，支持 `text` / `hex`。 |
...
| `set_kafka_record_key_from_offset_bytes` | 从 payload 固定 offset+length 截取字节，按 `text` 或 `hex` 写入 `packet.Meta.KafkaRecordKey`。 | `offset >= 0`；`length > 0`；`encoding` 必须是 `text` 或 `hex`；越界时终止当前 pipeline。 |
