...
本文有两类完全不同的吞吐口径，必须分开看：

1. **同进程处理上限（理论 0 丢包处理能力）**
   - 来自 `src/runtime` 的 micro-benchmark（`BenchmarkDispatchMatrix*`）。
   - 特点：不经过真实网卡与真实端到端链路，不统计网络丢包字段。
   - 含义：用于衡量服务在进程内的处理能力上限，可近似理解为“服务理论上能处理过来的数据量”。

2. **端到端 0 丢包上限（工程可达上限）**
   - 来自 `cmd/bench` 端到端测试，按 `loss_rate == 0` 取最大值。
   - 特点：受网卡带宽、内核网络栈、socket buffer、同机资源竞争等约束。
   - 含义：是线上更接近真实可达的“0 丢包吞吐上限”。

因此可以表述为：

- **服务的理论 0 丢包处理能力可到 ~1GB/s 量级**（同进程基准口径）；
- **当前环境下端到端 0 丢包上限在百 Mbps 量级**（受网卡/系统链路约束）。
...
2. 吞吐结论必须按两种口径拆分：
   - **同进程处理上限（理论处理能力）**：可到 ~1GB/s 量级；
   - **端到端严格 0 丢包上限（工程可达）**：当前环境约百 Mbps 量级。
3. 通过 bench 发包限速器与排空统计优化后，端到端观测恢复到合理区间：
   - 严格 0 丢包：112.04 Mbps；
   - 近零丢包：可稳定观测到 300+ Mbps。
4. 若目标是“严格 0 丢包且仍要 300+ Mbps”，建议下一步做：
...
> 该表是“同进程理论处理能力上限”，不是端到端 0 丢包网络上限。
