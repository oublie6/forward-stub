...
## 14. Docker / Kubernetes 最小部署

是的，你的理解是对的：
在 `CGO_ENABLED=0` 条件下构建出来的静态 Go 二进制，通常可以直接放到最小 Linux 基础镜像运行。
本仓库已补充最小部署文件：

- `Dockerfile`：distroless 最小运行时（直接拷贝外部已编译二进制）
- `.dockerignore`
- `deploy/k8s/`：`ConfigMap` + `Deployment` + `Service` + `kustomization.yaml`
...
```bash
docker build -t forword-stub:latest --build-arg BIN_PATH=bin/forword-stub .
```

> 如果你把二进制放在其它路径（例如 `dist/forword-stub`），改 `BIN_PATH` 即可。

### 14.3 本地容器运行

```bash
docker run --rm -p 19000:19000/udp forword-stub:latest
```
...
> 注意：`deploy/k8s/configmap.yaml` 中的 sender `remote` 目前是示例地址，请按你的实际下游地址修改。
